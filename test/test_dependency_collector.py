...
class TestDependencyCollector(unittest.TestCase):
    ...
    def test_get_ancillary_dependencies_finds_nearest_files_to_map_end_date(
        ...
        expected_ancillary_dependencies = [
            AncillaryInput("imap_hi_45sensor-cal-prod_20250101_v002.csv")
        ]
    ...
    def test_get_ancillary_dependencies_writes_energy_bin_edges_to_imap_dir(self, mock_query):
        ...
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                ultra_dep = (
                        ancillary_ultra_dir
                        / "imap_ultra_l2-energy-bin-group-sizes_20250924_v000.csv"
                )
                ...
                mock_query.assert_called_once_with(
                    table="ancillary", instrument="ultra"
                )
                expected_ancillary_dependencies = [
                    AncillaryInput(
                        "imap_ultra_l2-energy-bin-group-sizes_20250924_v000.csv"
        ...
        for descriptor, instrument_name, expected_glows_descriptor in test_cases:
            ...
            with self.subTest(str(descriptor)):
                ...
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    input_l2_path = (
                            tmpdir / f"imap_{instrument_name}_l2_map_20250101_v000.cdf"
                    )
            ...
            with self.subTest(str(descriptor)):
                ...
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    mock_query.return_value = [
                        ...
                        {
                            "file_path": f"imap_glows_l3e_{expected_glows_descriptor}_20250102_v000.cdf",
                            "start_date": "20250102",
                        },
                        {
                            "file_path": f"imap_glows_l3e_{expected_glows_descriptor}_20250103_v000.cdf",
                            "start_date": "20250103",
                        },
                    ]
                    ...
                    expected_inputs = [
                        ...
                        ScienceInput(
                            f"imap_glows_l3e_{expected_glows_descriptor}_20250103_v000.cdf"
                        ),
                        ScienceInput(
                            f"imap_{instrument_name}_l1c_pset_20250615_v001.cdf"
                        ),
    ...
    def test_get_sp_deps_returns_empty_list_for_spx_maps(self):
        ...
        lo_spx_sp_descriptor = create_map_descriptor(
            instrument=MappableInstrumentShortName.LO, principal_data="spx"
        )
