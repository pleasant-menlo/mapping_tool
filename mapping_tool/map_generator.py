...
def process(processor: ProcessInstrument, output_directory: Optional[Path] = None,
            output_file_name: Optional[str] = None) -> \
        list[Path]:
    downloaded_deps = processor.pre_processing()
    output_directory = output_directory or imap_data_access.config["DATA_DIR"]
    try:
    ...
    file_paths = processor.post_processing(results, downloaded_deps)
    ...
    #         match list(Path(output_directory).rglob("*.cdf")):
    #             case [file]:
